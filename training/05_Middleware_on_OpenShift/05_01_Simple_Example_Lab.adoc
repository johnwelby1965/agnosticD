...
[source,sh]
----
tree $HOME/agnosticd/ansible/roles_ocp_workloads/ocp4_workload_example
----
+
.Sample Output
[source,sh]
...
+
The variables in default/main.yml define the default variable values that will be shipped with your role.  They should be "sane defaults" that work in as many forseeable contexts as possible.  In this case, we have five variables. The first three variables are critical for AgnosticD to work, and the last two are unique to the role:
+
...
. Create a variables file that you can use to pass in new and updated variables as you run your workload.
Save this file in your role directory as `ocp4_workload_${GUID}_vars.yaml`.
+
[source,sh]
----
...
ocp4_workload_${GUID}_variable_2: "My variable 2"
----
+
[IMPORTANT]
====
...
[source,sh]
----
export TARGET_HOST="bastion.dev.openshift.opentlc.com" <1>
...
<2> Change to match your OSP username - the user must already exist. In OpenTLC, users will be dynamically created.
...
. Change the vars file you created earlier to reflect a parameter that you don't expect to change during this development session.
This file shoudl be saved in your role directory as `ocp4_workload_${GUID}_vars.yaml`.
+
[source,sh]
----
# You can set any variable
